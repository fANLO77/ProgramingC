...
int main() {
    ...
    for (int k = 0; k < numSizes; k++) {
        int n = sizes[k];
        int arr[n];
        generateArray(arr, n, 'r'); // Генерация случайного массива
        ...
        // SelectSort
        for (int j = 0; j < n; j++) arrCopy[j] = arr[j];
        selectSort_good(arrCopy, n, &M_select, &C_select);

        // BubbleSort
        for (int j = 0; j < n; j++) arrCopy[j] = arr[j];
        bubbleSort(arrCopy, n, &M_bubble, &C_bubble);

        // ShakerSort
        for (int j = 0; j < n; j++) arrCopy[j] = arr[j];
        shaker_sort(arrCopy, n, &C_shaker, &M_shaker);

        // InsertSort
        for (int j = 0; j < n; j++) arrCopy[j] = arr[j];
        insertSort(arrCopy, n, &M_insert, &C_insert);
